        ...
        public List<LiteratureModel> GetLiterature(int id, LiteratureFilterModel filters)
        {
            using (Logging.MethodCall(GetType(), new ParameterList { { "Id", id }, { "Filters", filters } }))
            {
                try
                {
                    ...
                    var query = estuary.EstuaryLiteratures.Select(i => i.Literature).Where(i => i.Available != "No").AsQueryable();
        ...
        public List<MapModel> GetMaps(int id, MapFilterModel filters)
        {
            using (Logging.MethodCall(GetType(), new ParameterList { { "Id", id }, { "Filters", filters } }))
            {
                try
                {
                    ...
                    var query = estuary.EstuaryMaps.Select(i => i.Map).Where(i => i.Available != "No").AsQueryable();
                    if (!string.IsNullOrWhiteSpace(filters?.Type))
                    {
                        query = query.Where(i => i.Type == filters.Type);
                    }
        ...
        public List<ImageModel> GetImages(int id, ImageFilterModel filters)
        {
            using (Logging.MethodCall(GetType(), new ParameterList { { "Id", id }, { "Filters", filters } }))
            {
                try
                {
                    ...
                    var query = estuary.EstuaryImages.Select(i => i.Image).Where(i => i.Available != "No").AsQueryable();
                    if (!string.IsNullOrWhiteSpace(filters?.Type))
                    {
                        query = query.Where(i => i.Type == filters.Type);
                    }
